 ...
 withConnectionTimeout:(NSTimeInterval)connectionTimeout
{
    if (!_isConnected && !_isConnecting) {
        ...
        if (_handshake) {
            ...
            // connection failed
            NSError *error = [NSError errorWithDomain: @"" code: 0 userInfo: @{NSLocalizedDescriptionKey: @"connection failed"}];
...
- (void) send:(SocketIOPacket *)packet
{
    ...
    if (![_transport isReady]) {
        ...
        if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIO:didSendMessage:)]) {
...
- (void) onConnect:(SocketIOPacket *)packet
{
    ...
    if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIODidConnect:)]) {
        ...
        switch (idx) {
            ...
            case 3: {
                ...
                if (packet.data && ![packet.data isEqualToString:@""]) {
                    if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIO:didReceiveMessage:)]) {
            ...
            case 4: {
                DEBUGLOG(@"json");
                if (packet.data && ![packet.data isEqualToString:@""]) {
                    if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIO:didReceiveJSON:)]) {
            ...
            case 5: {
                ...
                if (packet.data && ![packet.data isEqualToString:@""]) {
                    ...
                    if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIO:didReceiveEvent:)]) {
...
- (void) onDisconnect:(NSError *)error
{
    ...
    if ((wasConnected || wasConnecting)) {
        if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIODidDisconnect:disconnectedWithError:)]) {
...
{
    if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIO:onError:)]) {
...
- (void) connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response 
{
    ...
    if (IS_NOT_NULL(response) && [response respondsToSelector:@selector(statusCode)]) {
...
- (void) connection:(NSURLConnection *)connection didFailWithError:(NSError *)error 
{
    ...
    if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIO:onError:)]) {
        ...
        if (!IS_NOT_NULL(errorInfo)) {
            errorInfo = [NSDictionary dictionaryWithObject:[error localizedDescription] forKey:NSLocalizedDescriptionKey];
        }
    ...
    else if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIOHandshakeFailed:)]) {
...
{ 	
    ...
    if (connectionFailed) {
        ...
        // TODO: deprecated - to be removed
        else if (IS_NOT_NULL(_delegate) && [_delegate respondsToSelector:@selector(socketIO:failedToConnectWithError:)]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
