...
- (void)_HTTPHeadersDidFinish;
{
    ...
    [self _performDelegateBlock:^{
        if (IS_NOT_NULL(_delegate) && [self.delegate respondsToSelector:@selector(webSocketDidOpen:)]) {
...
- (void)_failWithError:(NSError *)error;
{
    dispatch_async(_workQueue, ^{
        if (self.readyState != SR_CLOSED) {
            _failed = YES;
            [self _performDelegateBlock:^{
                if (IS_NOT_NULL(self.delegate) && [self.delegate respondsToSelector:@selector(webSocket:didFailWithError:)]) {
...
- (void)_pumpWriting;
{
    ...
    if (_closeWhenFinishedWriting && 
        ...
        if (!_failed) {
            [self _performDelegateBlock:^{
                if (IS_NOT_NULL(self.delegate) && [self.delegate respondsToSelector:@selector(webSocket:didCloseWithCode:reason:wasClean:)]) {
            ...
            case NSStreamEventEndEncountered: {
                ...
                if (aStream.streamError) {
                    ...
                    if (!_sentClose && !_failed) {
                        ...
                        [self _performDelegateBlock:^{
                            if (IS_NOT_NULL(_delegate) && [self.delegate respondsToSelector:@selector(webSocket:didCloseWithCode:reason:wasClean:)]) {
